public class A2_P8_MostFrequent2_GARCIA_ASHLEY {

   public static String name = "GARCIA_ASHLEY";

// Using an array, this method must run in O(n) time.
//Using arrays, determine in LINEAR time the most frequent array element.


   public static int mostFrequent2(int n, int [] A) {


      int[] frequencyArray = new int[n+1]; //max size of array
      ...
      for (int i = 1; i <= n; i++)
      {
         frequencyArray[A[i]]++; //this increments the frequncy of the current number in the array

      if (frequencyArray[A[i]] > maxFrequency) {
         maxFrequency = frequencyArray[A[i]];
         mostFrequent = A[i]; // Update most frequent eleement if needed
      }
   }
